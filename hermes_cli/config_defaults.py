...
DEFAULT_CONFIG = {
    ...
    # Mobile is served by a separate, route-minimal listener. These values are
    # public identity-provider coordinates, not secrets. The listener remains
    # off unless hermes serve receives both --mobile-host and --mobile-port.
    "mobile": {
        # Externally visible HTTPS origin used to verify device DPoP proofs.
        # It must be pinned; request Host/X-Forwarded-* headers are never trusted.
        "public_url": "",
        # Host-side policy boundaries. Profile names are translated to opaque IDs
        # before they are exposed to mobile clients.
        "allowed_profiles": [],
        "allowed_scopes": [],
        # Bounded wall-clock budget for one mobile Hermes turn.  Mobile never
        # supplies this value; it is read only from the host configuration.
        "turn_timeout_seconds": 600,
        # False by default: cloudflared should be the only public-facing process.
        # A private VPN bind is an explicit operator choice and never accepts a wildcard.
        "allow_private_bind": False,
        "push_relay": {
            "url": "",
            # Credential is read only from HERMES_MOBILE_PUSH_RELAY_TOKEN.
        },
        "cloudflare_access": {
            "issuer": "",
            "audience": "",
