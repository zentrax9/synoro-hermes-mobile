...
def _mobile_device_summary(record: Any) -> dict[str, Any]:
    """Return the dashboard-safe projection of a mobile device record.

    Device public keys, access claims, and relay handles stay in the host store;
    the dashboard only needs enough opaque metadata to identify and revoke a
    phone.  In particular, never serialize ``DeviceRecord`` directly here.
    """
    ...
    return {
        ...
        "profiles": list(record.profile_allowlist),
        "scopes": list(record.scope_allowlist),
...
def _list_mobile_devices_sync() -> list[dict[str, Any]]:
    """Read the same host registry used by ``hermes mobile devices list``."""

    from hermes_cli.mobile_cmd import _configured_store

    with _configured_store() as store:
        return [_mobile_device_summary(record) for record in store.list_devices()]


def _revoke_mobile_device_sync(device_id: str) -> dict[str, Any]:
    """Revoke locally first, then best-effort reconcile the central relay."""

    from hermes_cli.mobile_cmd import _configured_relay, _configured_store

    with _configured_store() as store:
        record = store.revoke_device(device_id)
    relay_error: str | None = None
    try:
        relay = _configured_relay()
        if relay is not None:
            relay.revoke_device(record.push_handle)
    except Exception as exc:  # local revocation is already durable
        relay_error = str(exc) or exc.__class__.__name__
    result = _mobile_device_summary(record)
    result["relay_reconciled"] = relay_error is None
    if relay_error is not None:
        # Keep diagnostics actionable without exposing relay credentials or
        # request payloads to the browser.
        result["relay_warning"] = "Local revocation succeeded; push relay reconciliation is pending."
    return result


@router.get("/api/mobile/devices")
async def list_mobile_devices():
    """List mobile devices for the authenticated local dashboard operator."""

    try:
        return {"devices": await asyncio.to_thread(_list_mobile_devices_sync)}
    except SystemExit as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    except Exception as exc:
        _log.exception("Failed to list Hermes Mobile devices")
        raise HTTPException(status_code=503, detail="Mobile device registry unavailable") from exc


@router.post("/api/mobile/devices/{device_id}/revoke")
async def revoke_mobile_device(device_id: str):
    """Immediately revoke one device through the dashboard auth boundary."""

    try:
        return await asyncio.to_thread(_revoke_mobile_device_sync, device_id)
    except SystemExit as exc:
        raise HTTPException(status_code=503, detail=str(exc)) from exc
    except Exception as exc:
        # Do not distinguish an unknown opaque ID from another registry lookup
        # failure in the browser; the CLI remains the detailed operator path.
        _log.warning("Hermes Mobile device revocation refused: %s", exc)
